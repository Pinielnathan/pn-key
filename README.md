...
A standalone tool with three features. Every download from any of them is a tagged WAV — BPM and key embedded as standard ID3 `TBPM`/`TKEY` frames, the same metadata DJ software (Serato, rekordbox, Traktor) reads.
...
3. **Effects** — upload a vocal, pick a preset (Concert Hall, Telephone, Robot, Chipmunk, Deep Voice, Echo Chamber, Radio, Underwater, Haunted), download the processed result.

Standalone project, unrelated to any other repo on this machine — its own frontend, backend, and deploy story.

## Architecture

- **`frontend/`** — React + Vite + TypeScript + Tailwind. A static single-page app: file upload, controls, job-status polling, players, download links. No audio processing happens in the browser — it's a thin client for the backend.
- **`backend/`** — FastAPI (Python). One synchronous endpoint plus three async job types:
  ...
  - `separate`: shells out to `python -m demucs --two-stems vocals` (the `htdemucs` model) to produce `vocals.wav` and `no_vocals.wav` (used as the instrumental/beat). Both stems get tagged with the same detected BPM/key, since they're one song.
  - `effects`: runs a named preset — a chain of [`pedalboard`](https://github.com/spotify/pedalboard) effects (Reverb, Chorus, Distortion, Delay, PitchShift, filters, Bitcrush, Phaser) — over the upload. Presets are defined in `app/effects.py`; `GET /api/effects/presets` is the single source of truth the frontend reads from, nothing's hardcoded twice.
  - Every job re-analyzes its own output (or, for `separate`, the instrumental stem) and embeds the detected BPM/key via `mutagen` (`app/audio_ops.py:embed_metadata`) before marking itself done.
